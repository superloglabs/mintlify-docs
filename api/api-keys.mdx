---
title: "API Keys: Mint and Revoke Ingest Keys"
sidebarTitle: "API Keys"
description: "Use the Superlog management API to programmatically mint and revoke project ingest keys for automated provisioning and key rotation."
---

Ingest keys (`sl_live_…`) are the credentials your services use to push OTLP telemetry — traces, logs, and metrics — to Superlog. You can manage them from the dashboard or automate the entire lifecycle with the management API.
...
<ResponseField name="api_keys[].id" type="string (UUID)">
  Unique identifier for the key. Use this as `:id` when revoking.
</ResponseField>
...
```json
{
  "api_key": {
    ...
    "name": "Production ingest key",
    "key_prefix": "sl_live_ab",
    "plaintext": "sl_live_abcdefghijklmnopqrstuvwxyz"
  }
}
```

<ResponseField name="api_key.id" type="string (UUID)">
  Unique identifier. Use this to revoke the key later.
</ResponseField>
...
Immediately revokes an ingest key. Any services still using the revoked key will receive `401` errors on their next ingest request. The revocation is permanent and cannot be undone.

```bash
curl -X DELETE \
  https://api.superlog.sh/api/v1/projects/<projectId>/api-keys/<keyId> \
  -H "Authorization: Bearer sl_management_xxxx"
```
...
<ParamField path="id" type="string (UUID)" required>
  The key's UUID, as returned by the list or mint endpoints.
</ParamField>
...
Returns `404` if no active key with the given UUID exists in the project.

---

## Key rotation workflow

Use the following pattern to rotate an ingest key without dropping telemetry:
...
<Steps>
  ...
  <Step title="Revoke the old key">
    Call `DELETE /api/v1/projects/:projectId/api-keys/:id` with the old key's UUID once all services have migrated.
  </Step>
</Steps>
