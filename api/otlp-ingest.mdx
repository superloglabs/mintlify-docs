...
```
Authorization: Bearer sl_live_xxxx
```

Ingest keys are created per-project in **Settings → API Keys**. They are prefixed `sl_live_` and have no expiry by default; you can revoke them at any time.

<Note>
  Do not use a Personal Access Token (PAT) on ingest endpoints. PATs are for the MCP server only. Sending a `superlog_cli_*` or `sl_pat_*` token to an ingest endpoint returns `401`.
</Note>
